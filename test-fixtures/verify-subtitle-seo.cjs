...
http.get('http://localhost:4321/pdf/subtitle-converter', (res) => {
  let html = '';
  res.on('data', (c) => (html += c));
  res.on('end', () => {
    console.log('=== VERIFYING SUBTITLE CONVERTER SEO & SCHEMA ===');

    // Title
    const titleMatch = html.match(/<title>(.*?)<\/title>/);
    const title = titleMatch ? titleMatch[1] : '';
    console.log('Page Title:', title);
    if (!title.includes('Subtitle Converter')) {
      throw new Error(`Page title does not include Subtitle Converter: "${title}"`);
    }
    ...
    if (desc.length < 130 || desc.length > 165) {
      console.warn('WARNING: Meta description length outside 130-165 range');
    }
    ...
    if (!canonical.includes('/pdf/subtitle-converter')) {
      throw new Error(`Canonical does not point to /pdf/subtitle-converter: "${canonical}"`);
    }
    ...
    console.log('\n✓ SUBTITLE CONVERTER SEO & SCHEMA AUDIT PASSED 100%!');
  });
}).on('error', (err) => {
