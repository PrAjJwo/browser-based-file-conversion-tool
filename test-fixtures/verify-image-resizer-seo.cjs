...
http.get('http://localhost:4321/image/image-resizer', (res) => {
  let html = '';
  res.on('data', (c) => (html += c));
  res.on('end', () => {
    console.log('=== VERIFYING IMAGE RESIZER SEO & SCHEMA ===');

    // Title
    const titleMatch = html.match(/<title>(.*?)<\/title>/);
    const title = titleMatch ? titleMatch[1] : '';
    console.log('Page Title:', title);
    if (!title.includes('Image Resizer')) {
      throw new Error(`Page title does not include Image Resizer: "${title}"`);
    }
    ...
    if (!canonical.includes('/image/image-resizer')) {
      throw new Error(`Canonical does not point to /image/image-resizer: "${canonical}"`);
    }
    ...
    console.log('\n✓ IMAGE RESIZER SEO & SCHEMA AUDIT PASSED 100%!');
  });
}).on('error', (err) => {
