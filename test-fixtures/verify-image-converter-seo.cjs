...
http.get('http://localhost:4321/image/image-converter', (res) => {
  let html = '';
  res.on('data', (c) => (html += c));
  res.on('end', () => {
    console.log('=== VERIFYING IMAGE FORMAT CONVERTER SEO & SCHEMA ===');

    // Title
    const titleMatch = html.match(/<title>(.*?)<\/title>/);
    const title = titleMatch ? titleMatch[1] : '';
    console.log('Page Title:', title);
    if (!title.includes('WebP, PNG') || !title.includes('JPG Converter')) {
      throw new Error(`Page title does not include expected title: "${title}"`);
    }
    ...
    if (!canonical.includes('/image/image-converter')) {
      throw new Error(`Canonical does not point to /image/image-converter: "${canonical}"`);
    }
    ...
    console.log('\n✔ IMAGE FORMAT CONVERTER SEO & SCHEMA AUDIT PASSED 100%!');
  });
}).on('error', (err) => {
