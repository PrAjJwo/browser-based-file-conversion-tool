...
http.get('http://localhost:4321/image/social-resizer', (res) => {
  let html = '';
  res.on('data', (c) => (html += c));
  res.on('end', () => {
    console.log('=== VERIFYING SOCIAL IMAGE RESIZER SEO & SCHEMA ===');

    // Title
    const titleMatch = html.match(/<title>(.*?)<\/title>/);
    const title = titleMatch ? titleMatch[1] : '';
    console.log('Page Title:', title);
    if (!title.includes('Social Media Image Resizer')) {
    ...
    if (!canonical.includes('/image/social-resizer')) {
      throw new Error(`Canonical does not point to /image/social-resizer: "${canonical}"`);
    }
    ...
    console.log('\n✔ SOCIAL IMAGE RESIZER SEO & SCHEMA AUDIT PASSED 100%!');
  });
}).on('error', (err) => {
